...
describe('PingScreen', () => {
  it('shows the loading state while the request is in flight', () => {
    render(<PingScreen result={null} isLoading error={null} onRetry={vi.fn()} onSignOut={vi.fn()} onGoToProfile={vi.fn()} onGoToMeals={vi.fn()} onGoToMealLibrary={vi.fn()} onGoToActivity={vi.fn()} onGoToWorkouts={vi.fn()} onGoToProgress={vi.fn()} />);
  ...
  it('renders the synced user id and timestamp once the ping resolves', () => {
    ...
    render(<PingScreen result={result} isLoading={false} error={null} onRetry={vi.fn()} onSignOut={vi.fn()} onGoToProfile={vi.fn()} onGoToMeals={vi.fn()} onGoToMealLibrary={vi.fn()} onGoToActivity={vi.fn()} onGoToWorkouts={vi.fn()} onGoToProgress={vi.fn()} />);
